...
DrCacheDim::set_sets_ways($Hmax, (64, 8, 512, 16, 1024, 20, 16384, 64));
...
my $P = DrCacheDim::default_problem($fn);
$P->{cost} = DrCacheDim::get_real_cost_fun(\@cost);
$P->{jcfg} = "$jcfg";

#DrCacheDim::run_cachesim($P, $H0);
($H, $H0) = DrCacheDim::parallel_run($P, [$H, $H0]);
...
DrCacheDim::print_hierarchy($res->[-1]);

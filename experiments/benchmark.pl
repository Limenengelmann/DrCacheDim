...
my $lambda = 0.5;#0.1;
$tstamp = $tstamp . "-l$lambda" unless $lambda == 0.5;#. "-c100"; # . "-l0.1";
my $matmul_n = 128;
...
#Aux::hierarchy2latex($H_capw, "H_capw");
...
my $exe = {
    ...
    #"matmul_kij" => sub { return ("$Aux::ROOT/bin/matmul_kij $matmul_n", ""); },
    #"matmul_jki" => sub { return ("$Aux::ROOT/bin/matmul_jki $matmul_n", ""); },
    #"matmul_jik" => sub { return ("$Aux::ROOT/bin/matmul_jik $matmul_n", ""); },
    #"matmul_ikj" => sub { return ("$Aux::ROOT/bin/matmul_ikj $matmul_n", ""); },
    #mibench
...
foreach my $name (keys %$exe) {
    ...
    #my $cscale  = 0.18383808301615;
    DrCacheDim::set_cscale_lambda([$Hmin, $Hmax, $H0], $cscale, $lambda);
    DrCacheDim::run_cachesim($P, $H0);
    #DrCacheDim::run_cachesim($P, $H_capw);
    #DrCacheDim::run_cachesim($P, $Hmin);
    #DrCacheDim::run_cachesim($P, $Hmax);
    #my $max_mat = $H0->{MAT};
    #my $max_cost = $H0->{COST};
    #if ($name eq "capway") {
